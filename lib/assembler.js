...
Assembler.prototype.appTests = function() {
  if (this.tests) {
    ...
    var testSupport = mergeTrees(this.addonTreesFor('test-support').map(function(tree) {
      return rename(tree, function(relativePath) {
        return 'test-support' + path.sep + relativePath;
      });
    }, this));

    var testLoader = rename(find(testSupport, {
      include: ['test-support/test-loader.js'] }),
      function() {
        return this.options.outputPaths.testSupport.js.testLoader;
      }.bind(this)
    );

    // Evicit the test loader as it needs to be held in its own descriptor
    testSupport = rm(testSupport, 'test-support/test-loader.js');
    ...
    var testLoaderDesc = new TreeDescriptor({
      treeType: 'test-loader',
      tree: testLoader,
      name: this.testPath,
      packageName: this.project.pkg.name,
      nodeModulesPath: this.project.nodeModulesPath,
    ...
    this.cache.set(this.testPath, testLoaderDesc);
  }
};
