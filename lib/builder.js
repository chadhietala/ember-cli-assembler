...
Builder.prototype.dist = function() {
  return this.addonTreesFor('dist').map(function(tree) {
    var name = tree.name;
    tree = rename(tree, function(relativePath) {
      var parts = relativePath.split('/');
      if (parts.length !== 1 || relativePath === 'dep-graph.json') {
...
Builder.prototype.javascript = function() {
  ...
  var environmentFile = rename(find(packagerFiles, '__packager__/{environment.js}'), function(relativePath) {
    return relativePath.replace('__packager__/', self.name + '/config/');
  });

  appJavascript = mergeTrees([appJavascript, environmentFile]);
  ...
  return jsTrees.map(function(tree) {
    ...
    tree = rename(tree, function(relativePath) {
      var parts = relativePath.split('/');
      if (relativePath.slice(-1) !== '/' && parts[0] !== self.name) {
        if (parts.length > 1) {
          parts.shift();
          return parts.join('/');
        }
    ...
    var transpiledTree = new ES6Modules(tree, {
  ...
  }, this).concat(this.dist(), packagerFiles);
};
